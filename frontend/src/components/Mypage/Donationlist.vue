<template>
  <div class="my-donationlist-box container">
    ...
    <el-table
      ...
      <el-table-column prop="name" label="기부명"></el-table-column>
      <el-table-column prop="amount" label="기부 금액(B)"></el-table-column>
      <el-table-column prop="time" label="기부 일자"></el-table-column>
    </el-table>
  </div>
</template>
<script>
...
export default {
  setup(){
    ...
    onMounted(()=>{
      store.dispatch('Donation/findByEmail', state.user.email).then(({data})=>{
        data.forEach(function(item){
          state.tableData.push({
            compName: item.donate.comp.name,
            name: item.donate.name,
            amount: item.amount,
